...
def show_item_details(request, item_id):
    # Retrieve the details of the selected item based on the item_id
    # For simplicity, you can use a dictionary or a database to store and retrieve item details
    all_meals = {
         "categories": [
              {
                "idCategory": "1",
    ...
    selected_item = None
    for category in all_meals['categories']:
        if category['idCategory'] == item_id:
            selected_item = category
            break

    return render(request, 'item_details.html', {'selected_item': selected_item})